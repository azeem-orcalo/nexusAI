...
export class ModelsService {
  ...
  private getUseCaseIcon(useCase: string): string {
    const normalized = useCase.toLowerCase();

    if (normalized.includes("agent")) return "🤖";
    if (normalized.includes("reason")) return "🧠";
    if (normalized.includes("writ")) return "✍️";
    ...
    if (normalized.includes("coding") || normalized.includes("code")) return "💻";
    ...
    if (normalized.includes("open source")) return "🧩";
    return "✨";
  }
  ...
  async findOne(id: string) {
    ...
    const numericPrice =
      Number.parseFloat(selectedModel.pricePerMillion.replace(/[^0-9.]/g, "")) || 0;
    const outputPrice =
      numericPrice === 0 ? "$0.00 / 1M" : `$${(numericPrice * 1.35).toFixed(2)} / 1M`;
    ...
    return {
      ...
      overview: {
        ...
        maxOutput:
          selectedModel.contextWindow >= 1000000 ? "32,768 tokens" : "8,192 tokens",
